  ...
  WebServer = (function() {
    ...
    WebServer.prototype.setup_routing = function() {
      ...
      this.app.get('/clients', function(req, res, next) {
        if (__indexOf.call(Object.keys(req.query), "info") >= 0 && req.query.info === 'true') {
          return res.json(200, {
      ...
      this.app.post('/clients', function(req, res, next) {
        ...
        cl = _this.jsrv.add_client(client.name, client.url, client.template, client.cluster);
        return res.json(200, {
          name: client.name,
      ...
      this.app.del('/clients/:client', function(req, res, next) {
        return res.json(200, {
          clients: _this.jsrv.remove_client(req.params.client)
        });
      });
      this.app.get('/templates', function(req, res, next) {
        return res.json(200, {
          templates: _this.jsrv.list_templates()
        });
      });
      return this.app.get('/templates/:template', function(req, res, next) {
        var template_info;
        template_info = _this.jsrv.templates[req.params.template];
        if (template_info) {
          return res.json(200, template_info);
        } else {
          return res.json(404, {
            error: "The requested template doesn't exist"
          });
        }
      });
    };
